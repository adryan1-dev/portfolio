...
export function Contact() {
  return (
    <section id="contato" className="relative overflow-hidden px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-1/2 h-[26rem] w-[26rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/10 blur-[130px]"
      />
          ...
          {contactLinks.map(({ label, href, icon: Icon }) => (
            <li key={label}>
              <a
                ...
                aria-label={label}
                className="group flex h-20 w-20 flex-col items-center justify-center gap-1.5 rounded-2xl border border-border bg-secondary/40 transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/50 hover:shadow-[var(--glow-primary)] focus-visible:-translate-y-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:translate-y-0 sm:h-24 sm:w-24"
              >
                <Icon className="h-6 w-6 text-muted-foreground transition-colors duration-300 group-hover:text-primary sm:h-7 sm:w-7" />
